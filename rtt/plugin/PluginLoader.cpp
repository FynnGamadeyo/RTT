...
PluginLoader::PluginLoader() {
    log(Debug) <<"PluginLoader Created" <<endlog();
}
PluginLoader::~PluginLoader() {
    log(Debug) <<"PluginLoader Destroyed" <<endlog();
}
...
#if BOOST_VERSION >= 104600
        if ( 	(arg.string(), makeShortFilename(arg.filename().string()), kind, true) == false)
#else
...
void* PluginLoader::getHandler(std::string const& name) {
    MutexLock lock( listlock );
    vector<string> names;
    for(vector<LoadedLib>::const_iterator it= loadedLibs.begin(); it != loadedLibs.end(); ++it) {
        names.push_back( it->plugname );
